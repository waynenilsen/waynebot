...
describe("MessageCompose", () => {
  it("renders textarea and send button", () => {
    scenario();
    expect(
      screen.getByPlaceholderText("type a message... (@mention to ping)"),
    ).toBeInTheDocument();
  ...
  it("clears textarea after successful send", async () => {
    const s = scenario();
    const textarea = screen.getByPlaceholderText("type a message... (@mention to ping)");
  ...
  it("does not send whitespace-only messages", async () => {
    const s = scenario();
    await s.user.type(screen.getByPlaceholderText("type a message... (@mention to ping)"), "   ");
    expect(screen.getByText("send")).toBeDisabled();
  });
});
